...
def entrypoint() -> None:
    ...
    binary = here / ("agent-transcripts.exe" if os.name == "nt" else "agent-transcripts")
    if not binary.exists():
        sys.stderr.write(f"agent-transcripts binary not found at {binary}\n")
        sys.exit(1)
...
[[package]]
name          = "agent-transcripts-rust"
kind          = "crates"
crate         = "agent-transcripts"
path          = "packages/rust"
first_version = "0.0.1"
globs         = ["packages/rust/**", "LICENSE"]

[[package]]
name          = "agent-transcripts-py"
kind          = "pypi"
pypi          = "agent-transcripts"
path          = "packages/python"
first_version = "0.0.1"
build         = "maturin"
depends_on    = ["agent-transcripts-rust"]
...
def it_runs_the_tool(tmp_path):
    result = subprocess.run(
        ["agent-transcripts", "run", "--input", str(tmp_path / "in.json")],
        capture_output=True,
        text=True,
        check=True,
    )
...
**Type checker in CI** — `ty check agent-transcripts/` or `mypy agent-transcripts/` as a separate job. Type errors block merge.
...
typecheck:
    uv run ty check agent-transcripts/

test-unit:
    uv run pytest agent-transcripts/ -x -q
...
test-cov:
    uv run pytest --cov=agent-transcripts --cov-report=term-missing --cov-fail-under=85
...
| `security.yml` | `bandit -r agent-transcripts` |
...
kind       = "pypi"
path       = "."
globs      = ["agent-transcripts/**/*.py", "pyproject.toml", "uv.lock"]
